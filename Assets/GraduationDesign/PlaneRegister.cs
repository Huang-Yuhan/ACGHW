    ...
    public class PlaneRegister : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            for (int i = 0; i < x_max_count; i++)
            {
                ...
                {
                    uint index=(uint)(i * z_max_count + j);
                    data[index] = thisOneplaneData.position +
                                  thisOneplaneData.a_axis.normalized * i * particle_radius*2 +
                                  thisOneplaneData.b_axis.normalized * j * particle_radius*2; 
                    data[index]=transform.InverseTransformPoint(data[index]);
                }
            }
            GetComponent<ParticleSurfaceRender>().SetupData(data.ToList(), particle_radius);
        ...
        private void Update()
        {
            if (isRenderParticle)
            {
                ...
                GetComponent<ParticleSurfaceRender>().enabled = true;
            }else
            {
                GetComponent<MeshRenderer>().enabled = true;
                GetComponent<ParticleSurfaceRender>().enabled = false;
            }
        }
        ...
        private void OnDrawGizmosSelected()
        {
            ...
            Gizmos.DrawSphere(thisOneplaneData.position, 1f);
            //绘制两个轴
